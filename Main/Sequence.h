void moveRobotSequence(double distance)
{
    ...
    case 0:                     //Drive forward until edge of box
        ...
        if (distance <= 0)
        { //Does the US has a large distance input(when the US has a really large distance we have it return 0)
            if ((millis() - stopTimer) >= 300)
    ...
    case 6:                     //Drive forward until edge of box
        ...
        { //Does the US have a large distance
    ...
    case 12:                    //Drive forward until wall
        runMotors(targetAngle); //Run motors forward
        if (distance <= 5)
        { //Does the US has a small distance input
            if ((millis() - stopTimer) >= 100)
            {                         //Has 100ms passed with the US having a small distance
                robotDriveSequence++; //Go to next step in the drive sequence
                stopMotors();         //Stops the motors
                stopTimer = millis(); //Saving the time it's called
            }
